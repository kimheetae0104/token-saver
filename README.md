...
딱히 없습니다. 화면에서 직접 보이는 개입은 4슬롯 착수 게이트(아래 예시)뿐입니다 — 낭비 습관
코칭·의도 확인 넛지·효율 경고는 `systemMessage` 필드로 전환을 시도했으나 사용자 화면엔
뜨지 않는 것으로 실측 확인됐습니다(2026-08-10 — [알려진 제한사항](#알려진-제한사항) 참고),
현재로선 어시스턴트 자신의 컨텍스트에만 도달합니다. 아래는 4슬롯 게이트의 실제 동작
그대로의 예시입니다.
...
- **낭비 습관 코칭**(`habit_coaching.py`, UserPromptSubmit): 연결어 과다·과거 회고+재확인 결합·
  방향전환·긴 배경설명 같은 채팅 습관 패턴을 감지해 한 줄 피드백. `systemMessage` 필드로
  사용자 화면 노출을 시도했으나 뜨지 않는 것으로 실측 확인됨(2026-08-10, 아래 "알려진
  제한사항" 참고) — 현재는 어시스턴트 컨텍스트에만 도달.
...
- **매 턴 효율 상태 주입**: 프롬프트 제출마다(`--check`) 누적 토큰·캐시 적중률·비용·효율 점수를 한 줄로 계산해 Claude 자신의 컨텍스트에 주입 — 컨텍스트 비대·캐시 적중 저하는 같은 줄에 경고 추가, 재독 차단·트리머의 절감 추정치와 4슬롯 게이트 개입 횟수도 같은 줄/리포트에 합산. 전체 줄은 Claude 컨텍스트 전용입니다 — 경고(컨텍스트 비대·캐시 적중 저하 등)가 있을 때만 `systemMessage`로 사용자 화면 노출을 시도했으나 뜨지 않는 것으로 실측 확인됨(2026-08-10, 아래 "알려진 제한사항" 참고). `statusLine` 수동 설정이 여전히 절감 추정치까지 포함한 완전한 한 줄을 사용자 화면에 상시로 보는 유일한 확인된 방법입니다.
...
- **(2026-08-10 검증 완료 — 음성) `UserPromptSubmit` hook의 `systemMessage` 필드는 사용자
  화면에 뜨지 않습니다.** 앞선 두 차례 "검증"은 배포 갭(설치된 플러그인이 구버전) 때문에
  무효였으나, 이번엔 그 함정을 미리 배제했습니다 — `ps`로 이 세션을 소유한 claude 프로세스의
  기동 시각이 플러그인 캐시 갱신(0.3.6, 16:58:25 KST)보다 늦음(17:29:02 KST)을 먼저 확인하고,
  자식 MCP 서버가 실제로 `token-saver/0.3.6/mcp/server.py`를 실행 중임까지 확인한 뒤 사용자에게
  "테스트" 프롬프트를 보내달라고 요청 → 훅은 확실히 발화(어시스턴트 컨텍스트에 `⟢` 줄 주입 확인,
  2턴 연속)했지만 사용자는 두 턴 모두 화면에 아무것도 안 떴다고 확인. 원인은 코드 결함이 아니라
  **공식 문서 자체의 예외 조항으로 보임** — `code.claude.com/docs/en/hooks`는 `systemMessage`를
  "전체 hook 이벤트 공통, 사용자에게 표시"라고 일반적으로 서술하면서도, 바로 이어서
  `UserPromptSubmit`(과 `UserPromptExpansion`·`SessionStart`)은 예외로 "stdout이 Claude가 보는
  컨텍스트로 추가된다"고 별도로 명시한다 — 이 예외 조항이 `systemMessage` 필드에도 적용되는지는
  문서가 명시하지 않지만, 이번 실측(정확히 이 세 이벤트 중 하나에서만 실패)이 그 해석과 정확히
  들어맞는다. 결론: `intent_gate.py`·`habit_coaching.py`·`measure.py --check`의 `systemMessage`
  전환(`dffc140`) 자체는 안전하지만(컨텍스트 주입 효과는 유지) 애초 목표였던 "사용자 화면 노출"은
  달성 안 됨 — 되돌리지 않고 유지(부작용 없음, 컨텍스트 경로로는 여전히 유효). 아래 `statusLine`이
  여전히 유일하게 **확인된** 사용자 가시 경로입니다. "어시스턴트 자신의 상태 상기만으로 실제
  행동이 달라지는가"도 별개로 아직 실측되지 않은 미검증 가정입니다 — 후속 과제.
