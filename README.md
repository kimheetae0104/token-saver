...
![tests](https://img.shields.io/badge/tests-223%2F223_passing-brightgreen)
...
- **다단계 파이프라인 소배치 통위임 차단** (`ladder_gate.py`, PreToolUse matcher: `Agent`,
  2026-08-13)
  CLAUDE.md의 "생성→판정→측정 등 다단계 파이프라인을 서브에이전트 하나에 통위임 금지"
  규칙을 프롬프트 판단이 아니라 결정론 코드로 강제한다. 위임 프롬프트 텍스트에서 3개
  단계 카테고리(생성/판정/측정) 중 2개 이상 매치 + 명시적 순서 마커(뒤·다음·이후·
  그리고나서·→)가 함께 있으면 "다단계 신호"로 판정하고, 배치 크기가 20건 미만이거나
  불명시면(실험17: 소배치일수록 위임 오버헤드가 절감분을 역전, 최대 3.495배) 그 Agent
  호출을 1회 막는다 — 같은 프롬프트로 재시도하면 통과(강제 변경은 못 시키지만 "의식적
  재확인"은 강제, 기존 사다리 컨설트·4슬롯 게이트와 동일 설계). 단일 단계 위임(예:
  "PR diff 리뷰하고 판정해서 보고서 작성")처럼 CLAUDE.md 자신이 권장하는 패턴과는 순서
  마커 요구 조건으로 구분해 오탐을 줄였다(최종검토에서 발견해 강화). 근거: 실험10(오버헤드
  포함 시 절감 74.1%→11.5%로 축소)·실험17(N<20 소배치에서 baseline 대비 3.495배 손해).
  설계: `docs/superpowers/specs/2026-08-13-pipeline-batch-guard-design.md`. **실사용
  검증은 아직 0건**(유닛테스트만 통과) — 4슬롯 게이트(26차, 9건 실측)와 달리 오탐·누락률은
  미측정.
