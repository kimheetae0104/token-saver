...
## 🚨 근본 설계 오류 발견 — hook 출력은 사용자에게 절대 안 보임 (2026-08-08, 10차, 다음 세션 최우선)
...
1. README.md·CLAUDE.md·measure.py의 관련 주석에서 "사용자가 체감/가시화" 같은 표현을
   "어시스턴트 컨텍스트에 주입"으로 정정 — 벤더 주장 금지 원칙상 틀린 설명을 방치하면 안 됨.
2. 그래도 이 메커니즘에 남은 가치가 있는지 재평가: 어시스턴트 자신이 매 턴 상태를 상기하는 것만으로
   실제 행동(간결한 답변·조기 compact 등)이 달라지는지 실측 필요 — 지금까지 이 근거 없이
   "습관 형성"이라 주장해온 것 자체가 실측 안 된 가정이었음.
3. 사용자에게 실제로 보이게 하려면 다른 경로가 필요 — 예: statusLine(세션당 1개 제약 있음,
   README 알려진 제한사항 참고)이 유일하게 사용자 화면에 실제로 뜨는 경로. hook 기반 코칭을
