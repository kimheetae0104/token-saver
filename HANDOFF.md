...
## 23차 — 최근 3건 적대적 재검증 루프 + 잔여 버그 3건 수정 (2026-08-12)
사용자 요청("루프 돌려서 검증해")으로 20~22차의 3개 수정(ladder_gate 바레리스트 파싱,
check_citations.py, measure.py `_desc_tokens` 숫자 보존)을 각각 독립 sonnet 서브에이전트가
"반박을 시도"하는 적대적 재검증 1라운드 실행. 베이스라인 `pytest -q` 217/217 통과 확인 후
착수 — 3건 모두 실제로 반박에 성공(전부 python3 실행으로 직접 재현 확인, 추측 아님):

1. **`hooks/ladder_gate.py`** — `_extract_recommended_tier`가 (a) 리스트에 여러 블록이 있고
   첫 블록엔 text가 없는 경우(`val[0]`만 봐서 놓침), (b) `content`가 리스트가 아니라 문자열
   그대로 오는 경우, (c) `"content"` 래퍼 없이 단일 content-block dict가 그대로 오는 경우
   — 이 3가지 배선 변형을 처리 못 하고 조용히 `None` 반환(fail-open이라 게이트 자체는
   안 깨지지만 mismatch 재확인·usage 로깅이 죽음). `_text_from_content_blocks()` 헬퍼로
   통합 처리하도록 재작성. 회귀테스트 3건 추가.
2. **`measure.py`** — 22차가 만든 "숫자 토큰 항상 보존" 수정 자체가 **새 오탐을 만듦**:
   task/step 번호가 아니라 그냥 우연히 같은 한 자리 숫자로 끝나는 서로 무관한 설명 두 개가
   그 숫자 하나만으로 자카드가 0.667→0.700으로 문턱을 넘어 escalation_pair 오탐됨(alpha/beta
   예시로 재현). 재설계: 자카드 집합(`_desc_tokens`)은 숫자 예외 없이 원래대로 되돌리고,
   "Task/Step/Round/Wave N" 패턴만 정규식(`_TASK_NUM_RE`)으로 별도 추출해 번호가 다르면
   자카드 계산 전에 바로 False로 판정하는 좁은 앵커 방식으로 교체. 회귀테스트 2건 추가
   (우연한 숫자 공유 케이스, 여러 자리 Task 번호 케이스).
3. **`check_citations.py`** — 줄 단위로만 검사해서, 소프트 줄바꿈으로 인용이 두 줄에 걸치면
   (예: "실험 9\n후속 99") 뒷줄은 "실험" 접두사가 없어 매칭 자체가 안 되고 앞줄의 "실험 9"만
   단독으로 유효 판정돼 존재하지 않는 인용이 검사망을 완전히 빠져나감(스크립트 자신의 목적을
   정확히 무력화하는 케이스). 빈 줄로 구분되는 문단 단위로 줄을 합쳐 검사하도록 `find_citations`
   재작성(보고 줄번호는 문단 첫 줄 기준 근사치). 회귀테스트 1건 추가.

TDD로 6건 모두 수정 + 테스트. `pytest -q`: **223/223 PASS**(217+6). `check_citations.py`
재실행 OK(자기 자신도 통과). 버전 0.3.16→0.3.17(3건 모두 실동작 변경), README/README.en
배지 208→223. 적대적 재검증 1라운드로 종료(CLAUDE.md 상한) — 2라운드째는 진행 안 함.

## 24차 — 로드맵 4위 설계 검토 후 반려 (2026-08-12)
`suggest_tier` batch_size 자동 추정을 설계했다. `batch_size`가 실제로 의미하는 건 "이
서브에이전트 한 콜이 처리할 유사 반복 항목 수"([measure.py](measure.py) `suggest_tier()`
독스트링)인데, 이걸 결정론적으로 셀 방법을 두 갈래로 검토:
1. **같은 메시지 내 형제 Agent 호출 개수**(transcript_path 읽어 셈) — 유일하게 결정론적인
   신호지만 다른 질문에 답한다. "Agent를 N번 병렬 호출"은 "item당 1개씩 N개"지 "이 콜
   하나가 N건을 배치 처리"가 아니다. 게다가 이 신호가 정확히 맞아떨어지는 유일한 경우가
   CLAUDE.md "위임" 절이 이미 반대하는 워크플로우(item당 1개 위임 금지)라, 안 쓰는 패턴을
   지원하는 셈이 된다.
2. **Agent 프롬프트 본문에서 항목 수 세기**(번호 목록·파일 경로 개수 등) — 자유서술 텍스트
   분류로 직행, `ladder_gate.py` 자기 주석이 명시하는 실험9 함정(프로즈 채점 위양성·위음성
   실측) 그대로 재현.

가치는 우선순위표에서 이미 "낮음~중간 — 수동 입력이 저비용"으로 잡혀 있어, 두 갈래 다
위험 대비 이득이 없다. **구현하지 않기로 반려** — 사용자 확인 받고 종결
(`docs/superpowers/specs/2026-08-11-verification-and-roadmap-design.md` 우선순위표에
설계 결과 각주 추가). 로드맵 5~7위는 여전히 세션 누적 대기(액션 불가), 이번 검토로도
전제 안 바뀜.

## 25차 — 로드맵 5·7위 재확인: "액션 불가" 전제가 깨졌다 (2026-08-12)
사용자 요청("5~7위 재확인, 액션 가능한지")으로 `~/.claude/projects/.../*.jsonl` 전체
(90개 세션)를 직접 스캔. 결과: **5위·7위는 더 이상 블록 아님** — 6위만 그대로.

- **5위(`many_agents` 임계값 재보정)**: 마지막 재보정(2026-08-04, N=7, 분포
  `0/0/6/8/8/10/30`, many_agents=12)은 "8~10과 12 사이를 테스트하는 값이 아직 없다"며
  보류했다. 지금 전체 세션의 `n_agent_spawns`(`measure.parse_session()`)를 직접
  계산하니 새 outlier 5건: `e796ed47`=19, `d70ea518`=19, `c8016fa5`=41, `df5c36a0`=24,
  `694f2287`(이 세션 자신, `subagent-driven-development` 스킬 사용 구간)=22. 전부 12
  초과지만 8~10↔12 경계 자체를 테스트하는 11~15 구간 값은 여전히 없음 — 그래도 outlier가
  1건(30)에서 5건으로 늘어 "outlier 1건에 의존" 문제는 해소. **재설계 시 판단 필요**: 이
  5건이 30(실험8 의도적 벤치마크로 제외됨)처럼 "정상적인 배치 위임 패턴"(SDD 워크플로우 등)인지,
  아니면 진짜 과다 위임(낭비) 신호인지 구분이 먼저다 — 신호 자체(임계값 숫자)보다 이
  구분 기준을 세우는 게 핵심 설계 작업.
- **7위(병렬 haiku 겹침 매칭)**: `_subagent_records()`로 얻은 haiku 레코드의 start_ts/end_ts를
  세션별로 대조(다음 레코드 start_ts < 이전 레코드 end_ts = 겹침)한 결과, 세션 `e796ed47`에서
  **150건** 발견 — `Workflow` 툴(`pipeline`/`parallel`)로 동시에 여러 haiku를 spawn하는
  패턴에서 사실상 상시 발생. "실측 사례 없음"이 더 이상 사실이 아니다. `capture_failures()`의
  "다음 haiku 시작 전까지"로 매칭 폭을 좁히는 현재 로직(HANDOFF 11차 처방#2)이 동시성
  상황에선 전제(haiku가 순차 실행된다)가 깨져 오귀속 리스크가 실질적으로 있음 — 재설계 필요.
- **6위(`verify_fails.py` 이질적 judge 재검증)**: 이번 스캔 범위(`production_failures.jsonl`
  자동 탐지 신호)로는 확인 불가 — "위음성"(judge가 통과시켰지만 실제로 틀림)은 애초에
  이 로그가 잡는 신호 종류가 아니다. 그대로 블록 유지, 별도 확인 방법 필요.

**다음 세션에서 할 일**: 5위·7위 재설계 착수(이번 세션은 19.6시간·컨텍스트 98%라 세션위생
원칙상 새 세션 권장, 신중한 설계가 필요한 작업이라 더더욱). 5위는 "정상 패턴 vs 낭비"
구분 기준 브레인스토밍부터, 7위는 `capture_failures()`의 동시성 가정을 어떻게 고칠지
설계부터(예: haiku_starts를 "다음 시작"이 아니라 "겹치는 구간의 haiku 집합"으로 바꾸는 등,
아직 미확정).

## 26차 — 사용자 방향전환: 4슬롯 게이트 실사용 검증 (2026-08-12)
새 세션이 25차 지시대로 5위(`many_agents` 근접중복 신호) 브레인스토밍을 시작해 설계 스펙까지
썼으나(`docs/superpowers/specs/2026-08-12-many-agents-dup-signal-design.md`, 커밋
`136b47a` — 구현은 안 함, 재개 대상으로 보류), 사용자가 중간에 "루프 돌려서 검증하고
신뢰도 향상시키고 토큰 소모량 검증하면서 4슬롯 효과를 확인하라"고 방향을 바꿔 2026-08-09
설계한 4슬롯 하드게이트(`hooks/intent_gate.py`+`hooks/prompt_gate.py`)의 실사용 검증으로
전환했다. 합성 벤치마크가 아니라 **배포 이후 실제 트립 전수(9건, 전 프로젝트 합산
`gate_events/*.jsonl`)**를 스캔:

- **신뢰도**: 9/9(100%) 트립 모두 재시도 전 의도·제약·성공기준 설명 텍스트를 실제로 생성함
  (회피 사례 0건). 8/9는 즉시 통과(더블트립 버그 없음), 1건(`cf26dbe1`)은 echo 직후 세션
  자체가 끝나 판정 불가(게이트 결함 아님). 트립 3건은 첫 시도가 `AskUserQuestion`이었는데,
  이건 "게이트가 불필요하게 질문까지 막은 버그"가 아니라 — CLAUDE.md 원칙("애매하면
  echo 후 확인")대로 echo 없이 질문으로 직행한 진짜 위반을 정확히 잡은 것으로 확인(설계
  검증됨, 수정 불필요).
- **토큰 오버헤드**: 트립당 평균 버려지는 output ≈1,302tok(막힌 첫 시도) + echo·재시도
  output ≈1,093tok. 9건 누적 ≈21.5K output tok(배포 후 3일, 여러 프로젝트 합산) — 절대량
  작음.
- **intent_gate 오탐**: 9/9 트립 전부 설계대로 `SHORT_INTENT_MAX=3` 단어 이하에서만 발생.
  그중 1건(`흠 맞게 구현된거지?`, `23be5782`)은 "구현"이 ACTION_WORDS에 걸려 트립됐지만
  실제론 질문(과거 시제)이지 지시가 아니었음 — 정규식이 시제/어투를 구분 못 하는 경미한
  오탐(9건 중 11%). 막힌 동작이 read-only Bash라 실질 피해 없음, 수정은 보류(저비용·저빈도,
  증거 대비 수정 정당화 안 됨).
- **intent_gate 누락**: 자체 프로젝트 91개 세션에서 교정 마커(`아니`/`틀렸` 등) 28건 중
  26건은 자동화 skill 프리앰블에 마커 문자열이 우연히 섞인 `measure.py` 쪽 교정비율
  프록시 자체의 잡음(intent_gate와 무관 — 별도 이슈로만 남김, 이번 스코프 아님). 진짜
  한국어 교정 발화는 2건: 하나는 2026-08-05(게이트 배포 08-09 이전, 무관), 하나(`eec98b1e`)는
  직전 지시가 11단어라 애초에 `SHORT_INTENT_MAX=3` 설계 범위 밖(의도된 갭, 결함 아님).

**결론**: 4슬롯 게이트 두 훅 다 설계 의도대로 실사용에서 정확히 동작 중 — 이번 검증에서
코드를 바꿀 만한 근거는 안 나왔다(증거 없는 "신뢰도 향상"은 rework 리스크라 보류).

**다음 세션에서 할 일**: 로드맵 5위 스펙(위 커밋)이 브레인스토밍 단계에서 중단된 채
남아있음 — 사용자 재개 시 `writing-plans`부터 이어가면 됨. 7위는 여전히 설계 착수 전.

## 27차 — 다단계 파이프라인 소배치 통위임 하드 게이트 + 성능검증 지표 공식 채택 (2026-08-13)
사용자가 "주간 토큰 33%가 절약인지" 질문에서 출발해 "토큰세이버가 실제로 효과 있는지",
이어서 "무조건 이득이 되게, 범용적으로(초보자가 평문만 써도) 만들어달라"고 요청. 브레인스토밍
결과 "평문→효율 프롬프트 자동 번역 레이어"는 명시적으로 범위 밖(LLM 호출 필요=AI-YAGNI 위반,
결정론 불가)으로 기각하고, 대신 CLAUDE.md에 이미 문서화만 돼 있던 "다단계 파이프라인 통위임
금지" 규칙 — 실험10·17로 실측 손해까지 확인됐지만 프롬프트 기억에만 의존하던 규칙 — 을
`hooks/ladder_gate.py`에 결정론 코드로 고정하는 것으로 스코프를 좁혔다(사용자 승인).

superpowers 전체 체인(브레인스토밍→스펙→계획→subagent-driven-development→최종검토→수정→
push)을 실행:
- 스펙(`docs/superpowers/specs/2026-08-13-pipeline-batch-guard-design.md`, `c0ec09a`)·
  계획(`2026-08-13-pipeline-batch-guard.md`, `2a0a0bb`) 작성.
- Task 1(`0052864`): `ladder_gate.py`에 생성/판정/측정 3개 카테고리 정규식 + 배치크기
  정규식(`BATCH_SMALL_THRESHOLD=20`) 기반 게이트 추가 — 기존 티어 불일치 확인과 동일하게
  1회 deny 후 재시도 시 통과하는 패턴 재사용.
- Task 2(`43a43d0`): `docs/TOKEN-GUIDE.md`에 "캐시절감 $·효율점수"를 세션 성과 판정
  공식 지표로 명문화(신규 로직 없음, 기존 계산 공식 채택 선언).
- 최종 브랜치 리뷰(opus)에서 실제 버그 1건 발견: `measure.py`의 `ladder_gate_events`
  리더가 이벤트 타입을 안 걸러서 새 `pipeline_batch_flagged` 이벤트가 사다리 컨설트
  집계·비용비교 워크를 오염시킴 — `event` 필터 파라미터 추가로 수정(`3b5bc18`).
  플랜충돌성 발견 1건(2개 카테고리 매치만으로는 CLAUDE.md 자신이 권장하는 단일단계
  검증위임 패턴을 오탐)은 사용자에게 직접 질의 → 순서 마커(`_SEQUENCE_RE`) AND 조건
  추가로 강화하는 쪽으로 결정, 같은 커밋에 반영.
- 레포 관례(브랜치 이력 없음, 항상 main 직접 커밋) 확인 후 별도 브랜치 없이 진행,
  테스트 39/39(ladder_gate)+38/38(measure_refactor) 통과 확인 후 `git push origin main`
  (`4ec33f9..3b5bc18`).
- 문서 최신화(이번 항목): README.md/README.en.md에 새 게이트 설명 추가(**실사용 검증
  0건, 유닛테스트만** 명시), `plugin.json`/`marketplace.json` 버전 `0.3.17→0.3.18`.

**중요 한계(사용자에게도 직접 설명함)**: 이 게이트는 Agent 위임 호출 시점에만 개입하므로
세션 전체 토큰 페이스(turn당 누적 tok/h)는 구조적으로 못 움직인다 — `measure.py`의 페이스
지표는 세션 간 비교 기능이 없는 단일세션 실시간 추정치. "전체 사용량이 줄었다"는 근거는
없고, 이 게이트가 실제로 줄이는 건 "다단계 파이프라인을 소배치로 통위임하는" 특정 패턴의
오버헤드뿐. 최종검토에서 나온 Minor 5건(배치숫자 첫값만 파싱/영어 미지원/`--mark-consulted`가
`pipeline_batch_acknowledged` 리셋/게이트 체이닝 미문서화/TOKEN-GUIDE $ 우선 프레이밍)은
전부 deferred — 워크스페이스 삭제로 텍스트는 git 히스토리에 없음, 이 항목이 유일한 기록.

**다음 세션에서 할 일**: 이 새 게이트의 실사용 트립을 26차 4슬롯 게이트 검증과 같은 방식으로
`ladder_gate_events/*.jsonl`에서 `event=="pipeline_batch_flagged"` 스캔해 오탐/누락률
실측 — 아직 배포 직후라 데이터 없음, 몇 세션 누적 후 재확인. 로드맵 5위 스펙(26차 기록,
`136b47a`)도 여전히 브레인스토밍 단계에서 대기 중.
...
4. 로드맵 1~4위 전부 종결(4위는 구현이 아니라 반려로 종결, 24차 참고). **5·7위는 25차에서
   블록 해제됨 — 다음 우선 착수 대상.** 6위만 여전히 액션 불가(실전 위음성 사례 0건).
